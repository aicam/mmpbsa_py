...
def make_mutant_trajectories(INPUT, FILES, rank, cpptraj, 
   ...
   # GBNSR6 generate files
   if INPUT['gbnsr6run']:
      for structure in ['complex', 'receptor', 'ligand']:
         assosiated_file = FILES.mutant_complex_prmtop if structure == 'complex' else \
            FILES.mutant_receptor_prmtop if structure == 'receptor' else \
            FILES.mutant_ligand_prmtop
         structure_traj = Trajectory(assosiated_file, pre + 'mutant_%s.%s.%d' % (structure, trj_suffix, rank), cpptraj)
         structure_traj.Setup()
         print("processing in gbnsr6 ", structure, ' ', structure_traj.processed_frames)
         structure_traj.OuttrajGBNSR6(structure + "_mutant", structure_traj.processed_frames, FILES.temp_directory, rank)
         structure_traj.Run()

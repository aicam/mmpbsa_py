...
from MMPBSA_mods.amber_outputs import (QHout, NMODEout, QMMMout, GBout, PBout,
                                       ...
                                       SingleTrajBinding, MultiTrajBinding, GBNSR6out)
# GBNSR6
from MMPBSA_mods.calculation import (CalculationList, EnergyCalculation,
                                     ...
                                     PrintCalc, LcpoCalc, MolsurfCalc, GBNSR6Calculation)
...
class MMPBSA_App(object):
   ...
   def _load_calc_list(self, prefix, mutant, parm_system):
      ...
      progs = {'gb' : self.external_progs['mmpbsa_py_energy'].full_path,
               ...
               # GBNSR6
               'gbnsr6': self.external_progs['gbnsr6'].full_path
              }
      ...
      if self.INPUT['gbnsr6run']:
          mutant_postfix = ''
          if prefix.__contains__("mutant"):
              mutant_postfix += "_mutant"
          self.calc_list.append(PrintCalc('\nBeginning GBNSR6 calculations with %s' %
                                          progs['gbnsr6']), timer_key='gbnsr6')

          c = GBNSR6Calculation(progs['gbnsr6'], parm_system.complex_prmtop,
                                'complex',
          ...
          c = GBNSR6Calculation(progs['gbnsr6'], parm_system.receptor_prmtop,
                                'receptor',
                                '%s' + mutant_postfix + '.inpcrd.%d.%d',
          ...
          c = GBNSR6Calculation(progs['gbnsr6'], parm_system.ligand_prmtop,
                                'ligand',
                                '%s' + mutant_postfix + '.inpcrd.%d.%d',
   ...
   def process_input(self):
      ...
      # GBNSR6
      self.INPUT['gbnsr6run'] = False
      if self.INPUT['igb'] == 66:
          self.INPUT['igb'] = 5
          self.INPUT['gbnsr6run'] = True
   ...
   def parse_output_files(self):
      ...
      # gbnsr6
      triggers = ('nmoderun', 'gbrun', 'pbrun', 'rismrun_std', 'rismrun_gf', 'gbnsr6run')
      outclass = (NMODEout, GBClass, PBout, RISM_Std, RISM_GF, GBNSR6out)
      outkey = ('nmode', 'gb', 'pb', 'rism std', 'rism gf', 'gbnsr6')
      basename = ('%s_nm.out', '%s_gb.mdout', '%s_pb.mdout', '%s_rism.mdout',
                 '%s_rism.mdout', '%s_gb.mdout')
      # gbnsr6 since the FILES variable is not passed to outparser classes, we added a new INPUT
      # with same keys as self.INPUT and appended the path into it
      passing_INPUT = self.INPUT.copy()
      passing_INPUT['temp_directory'] = FILES.temp_directory
      for i, key in enumerate(outkey):
         ...
         if not INPUT['mutant_only']:
            self.calc_types[key] = {'complex' : outclass[i](self.pre +
                           basename[i] % 'complex', passing_INPUT, self.mpi_size,
                           self.using_chamber)}
            if not self.stability:
               self.calc_types[key]['receptor'] = outclass[i](self.pre +
                           basename[i] % 'receptor', passing_INPUT, self.mpi_size,
                           self.using_chamber)
               self.calc_types[key]['ligand'] = outclass[i](self.pre +
                           basename[i] % 'ligand', passing_INPUT, self.mpi_size,
                           self.using_chamber)
         ...
         if INPUT['alarun']:
            self.calc_types['mutant'][key] = {'complex' :
                  outclass[i](self.pre + 'mutant_' + basename[i] % 'complex',
                           passing_INPUT, self.mpi_size, self.using_chamber)}
            if not self.stability:
